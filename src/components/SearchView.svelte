...
<div class="flex-1 flex flex-col h-screen overflow-hidden bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-neutral-100 transition-colors">
  ...
  {#if !query.trim()}
    <!-- Centered Hero State (Positioned Around Middle of the Screen) -->
    <div
    ...
    >
      ...
      <div in:fly={{ y: 12, duration: 300 }} class="w-full max-w-2xl space-y-4">
        ...
        <div class="relative group">
          <Search class="w-5 h-5 text-neutral-400 dark:text-neutral-500 absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none group-focus-within:text-blue-500 transition-colors" />
          <input
            type="text"
            bind:value={query}
            placeholder="Search questions, policies, specifications, and architecture decisions..."
            class="w-full pl-12 pr-12 py-3.5 bg-white dark:bg-neutral-900 hover:bg-neutral-50 dark:hover:bg-neutral-850/80 focus:bg-white dark:focus:bg-neutral-900 border-2 border-neutral-200 dark:border-neutral-700 focus:border-blue-500 dark:focus:border-blue-500 rounded-2xl text-sm md:text-base text-neutral-900 dark:text-neutral-100 placeholder:text-neutral-400 dark:placeholder:text-neutral-500 focus:outline-hidden transition-all shadow-md focus:shadow-lg focus:shadow-blue-500/5"
          />
          {#if query}
            <button
              type="button"
              onclick={() => (query = '')}
              class="absolute right-3.5 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-neutral-700 dark:hover:text-neutral-200 p-1 rounded-full cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800"
              title="Clear search"
            >
              <X class="w-4 h-4" />
            </button>
        ...
        <div class="pt-2 text-center">
          <div class="text-[11px] text-neutral-400 dark:text-neutral-500 mb-2 flex items-center justify-center gap-1.5">
  ...
  {:else}
    ...
    <div in:fade={{ duration: 180 }} class="flex-1 flex overflow-hidden">
